...
public class XProcFunctionLibrary implements FunctionLibrary {
    private XProcRuntime runtime;
    private IterationPosition iterationPosition;
    private IterationSize iterationSize;
    private StepAvailable stepAvailable;
    private ValueAvailable valueAvailable;
    ...
    public XProcFunctionLibrary(XProcRuntime runtime, XStep forStep) {
    ...
    */
    ...
    protected void init() {
        functionTable = new HashMap<String,StandardFunction.Entry>(6);
        StandardFunction.Entry e;
        e = register("system-property", SystemProperty.class, 0, 1, 1, Type.ITEM_TYPE, StaticProperty.EXACTLY_ONE);
        StandardFunction.arg(e, 0, BuiltInAtomicType.STRING, StaticProperty.EXACTLY_ONE, null);
        e = register("step-available", StepAvailable.class, 0, 1, 1, Type.ITEM_TYPE, StaticProperty.EXACTLY_ONE);
        StandardFunction.arg(e, 0, BuiltInAtomicType.STRING, StaticProperty.EXACTLY_ONE, null);

        e = register("value-available", ValueAvailable.class, 0, 1, 2, Type.ITEM_TYPE, StaticProperty.ALLOWS_ONE_OR_MORE);
        StandardFunction.arg(e, 0, BuiltInAtomicType.STRING, StaticProperty.EXACTLY_ONE, null);
        StandardFunction.arg(e, 1, BuiltInAtomicType.BOOLEAN, StaticProperty.ALLOWS_ZERO_OR_ONE, null);

        e = register("iteration-position", IterationPosition.class, 0, 0, 0, Type.ITEM_TYPE, StaticProperty.ALLOWS_ZERO);
        //StandardFunction.arg(e, 0, BuiltInAtomicType.STRING, StaticProperty.ALLOWS_ZERO, null);
        e = register("iteration-size", IterationSize.class, 0, 0, 0, Type.ITEM_TYPE, StaticProperty.ALLOWS_ZERO);
        //StandardFunction.arg(e, 0, BuiltInAtomicType.STRING, StaticProperty.ALLOWS_ZERO, null);
        e = register("base-uri", BaseURI.class, 0, 0, 1, Type.NODE_TYPE, StaticProperty.ALLOWS_ZERO_OR_ONE);
        ...
        stepAvailable = new StepAvailable(runtime, step);
        valueAvailable = new ValueAvailable(runtime, step);

        // FIXME: HACK!
        if (step instanceof XForEach || step instanceof XViewport || step instanceof XPipeline || step instanceof XRootStep) {
            iterationPosition = new IterationPosition(runtime);
            iterationSize = new IterationSize(runtime);
        } else {
            XStep parent = step.getParent();
            while (parent != null && ! (parent instanceof XForEach || parent instanceof XViewport || parent instanceof XPipeline)) {
                parent = parent.getParent();
            }
            if (parent == null) {
                throw new XProcException("This can't happen: parent is null in XProcFunctionLibrary?");
            }
            XProcFunctionLibrary lib = parent.getFunctionLibrary();
            iterationPosition = lib.getIterationPosition();
            iterationSize = lib.getIterationSize();
        }

    }

    public IterationPosition getIterationPosition() {
        return iterationPosition;
    }

    public IterationSize getIterationSize() {
    ...
    public boolean isAvailable(StructuredQName functionName, int arity) {
        if (functionName.getNamespaceURI().equals(XProcConstants.NS_XPROC)) {
    ...
    public Expression bind(StructuredQName functionName, Expression[] staticArgs, StaticContext env)
        ...
        if (uri.equals(XProcConstants.NS_XPROC)) {
            ...
            if ("iteration-position".equals(local)) {
                f = iterationPosition;
            } else if ("iteration-size".equals(local)) {
                f = iterationSize;
            } else if ("step-available".equals(local)) {
                f = stepAvailable;
            } else if ("value-available".equals(local)) {
                f = valueAvailable;
            } else {
                try {
    ...
    */
    ...
    public void setIterationPosition(int pos) {
        iterationPosition.setPosition(pos);
    }

    public void setIterationSize(int size) {
        iterationSize.setSize(size);
    }
}
//
